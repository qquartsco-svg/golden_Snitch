...
- **테스트**: 패키지 루트에서 `python3 -m pytest tests/ -q` → **21 passed** (현재).  
...
`0.1.3` — `hover_margin_hint` safety 연결, `Sensory -> DCF -> Battery -> StubDriver` end-to-end 예제 추가.

## 테스트

```bash
cd _staging/Drone_Control_Foundation   # 00_BRAIN 모노레포 기준
python3 -m pytest tests/ -q            # 기대: 12 passed
python3 examples/run_tam_dcf_stub_adapter.py
python3 examples/run_sensor_dcf_battery_stub.py
python3 examples/run_sensory_tam_dcf_chain.py
python3 -m pytest tests/ -q            # 기대: 21 passed
```
